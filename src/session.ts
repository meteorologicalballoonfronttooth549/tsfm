...
export class LanguageModelSession {
  ...
  constructor(
    ...
    const toolArrPtr =
      tools.length > 0 ? koffi.as(toolPtrs, "void **") : null;
  ...
  static fromTranscript(
    ...
    const toolPtrs =
      tools.length > 0 ? koffi.as(rawPtrs, "void **") : null;
    ...
    const session = Object.create(
      LanguageModelSession.prototype,
  ...
  get isResponding(): boolean {
    ...
    return getFunctions().FMLanguageModelSessionIsResponding(
      this._ptr,
    ) as boolean;
  }
  ...
  async *streamResponse(
    ...
    const streamRef = fn.FMLanguageModelSessionStreamResponse(
      this._ptr,
      prompt,
      optionsJson,
    );
  ...
  private _runResponseCallback(
    callC: (cbPtr: unknown) => unknown,
  ): Promise<string> {
  ...
  private _runStructuredCallback(
    ...
    return new Promise<GeneratedContent>((resolve, reject) => {
      ...
      const cbPtr = koffi.register((...args: StructuredCbArgs) => {
        ...
        if (status !== 0) {
          const msg = getFunctions().FMGeneratedContentGetJSONString(contentRef) as string | null;
  ...
  private _respondWithSchema(
    ...
    return this._runStructuredCallback((cbPtr) =>
      fn.FMLanguageModelSessionRespondWithSchema(this._ptr, prompt, schema._ptr, optionsJson, null, cbPtr),
    );
  }

  private _toAppleSchemaFormat(
  ...
  private _respondWithJsonSchema(
    ...
    return this._runStructuredCallback((cbPtr) =>
      fn.FMLanguageModelSessionRespondWithSchemaFromJSON(this._ptr, prompt, schemaJson, optionsJson, null, cbPtr),
    );
  }
}
